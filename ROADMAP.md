...
- **Dernière mise à jour** : 2026-07-10 (Sprint 21 — **Maintenance technique D59 + D60**, sprint de maintenance au sens de `RUNBOOK.md` §8.2 (preuve objective = journaux CI du run `29094492121`), ouvert par décision utilisateur explicite (demande d'exécution du sprint + approbation du diff D60) SANS sortir du mode maintenance : **21.0** diff D60 APPLIQUÉ à `CLAUDE.md` (TSan bloquant, six jobs CI — commit dédié citant la décision, précédent D54) ; **21.1** flake d'isolation D59 CORRIGÉ — helper partagé `tests/support/TempPath.hpp` (nom = préfixe + PID + compteur atomique + tick : le PID sépare les processus ctest parallèles, cause racine de « SQLite exec: database is locked »), test rouge d'abord (TempPathUnit : 2 tests échouaient sur la transposition fidèle de l'ancien schéma tick-seul), fixtures SQLite migrées (DbLogger unit/intégration, StateStore) PUIS généralisation par audit aux 7 fixtures CSV/JSON/log portant le même motif — plus aucune occurrence de `time_since_epoch` dans `tests/*.cpp` ; stress local `ctest -R 'DbLogger|StateStore' --repeat until-fail:30 -j16` vert. 814 → **818/818 verts localement** ; goldens byte-identiques, `config/prod.json` intact, verrou live intact. Validation CI du SHA final : déclenchée par le push, à vérifier avant toute fusion — règle §8.4)
- **Mise à jour précédente** : 2026-07-10 (Sprint 20 — **Mode maintenance**, décision utilisateur (e), prise après une revue pré-sprint indépendante (verdict NO-GO : aucun périmètre exécutable tant que le choix (e)/(a'')/(r'') — réservé à l'utilisateur — n'était pas rendu ; constats factuels contre-vérifiés un par un sur le dépôt, tous exacts). Sprint SANS modification de code : **20.1** baseline LOCALE documentée (Linux paquets système sans vcpkg, build Debug -Werror sans warning, `ctest -N` = 814 = tableau de bord — AUCUNE dérive, 814/814 verts LOCALEMENT ; la CI GitHub du SHA de tête est en revanche ROUGE — flake d'isolation DbLogger, D59 : le sprint n'est PAS entièrement validé tant qu'une CI complète n'est pas verte) ; **20.2** politique de maintenance écrite (`RUNBOOK.md` §8 : périmètre autorisé, dépendances au cas par cas sur alerte sécurité/rupture de build, preuve objective documentée avant tout correctif — test rouge lorsqu'applicable —, règle CI « aucun merge/clôture déclarée verte sans CI complète verte du SHA concerné », baseline locale mensuelle minimum consignée dans un journal de maintenance (issue GitHub dédiée), sortie du mode par décision utilisateur uniquement). D58-D60 consignées (contrat cross-feed ; flake DbLogger en CI ; contradiction TSan de CLAUDE.md). Aucun golden touché, `config/prod.json` intact, verrou live intact)
- **Sprint courant** : 🔬 **SPRINT 22 — Chantier (r'') : indices dérivés d'options (SKEW / VVIX)** — ouvert le 2026-07-11 par **décision utilisateur explicite** (sortie du mode maintenance au sens de `RUNBOOK.md` §8.5, conditions 1-6 : décision consignée ici, items numérotés et critères d'acceptation ci-dessous, dépendances/décisions préalables rendues par l'utilisateur à l'ouverture). Périmètre décidé par l'utilisateur : **données** = indices CBOE dérivés de la surface d'options, `^SKEW` (prix du risque de queue, 1990+) et `^VVIX` (vol de la vol, ~2007+) ; **fournisseur/budget/licence** = pipeline Yahoo v8 existant (`scripts/export_total_return.py` inchangé — coût 0, même canal et mêmes conditions que ^VIX/^VIX3M des Sprints 14/19) ; **hypothèses** (jugées SÉPARÉMENT) = (a) régime SKEW : queue chère (SKEW haut vs sa propre médiane glissante) → cash, (b) régime VVIX : idem sur la vol de la vol ; **benchmark et DoD INCHANGÉS** (alpha absolu vs B&H net de coûts, OOS, 3 pavages, grille resserrée, MC size-aware, confirmation hors-protocole avant toute adoption — D43). Moteur : `VixRegimeBacktester`/`VixRegimeWalkForward` (Sprint 14) réutilisés TELS QUELS (génériques : {actif tradé, indice signal}, seuil × médiane glissante) — aucun nouveau moteur, aucun golden touché. Baseline à l'ouverture : **818/818 verte** (`ctest -N`=818, aucune dérive) et CI du SHA `374f64c` (run 29134734275) TERMINÉE VERTE le 2026-07-11 — caveat §8.4 du Sprint 21 LEVÉ. Items :
  - [ ] **22.0** Porte de données : export `SKEW_max.csv` + `VVIX_max.csv` (script inchangé, END_DATE figée 2026-07-02), gate qualité (dates strictement croissantes, close > 0, Adj == Close ATTENDU — indices, hors `auditTotalReturnCsv`), exceptions `.gitignore`. Critère : les deux CSV présents, propres, commités.
  - [ ] **22.1** Verdict OOS SKEW verrouillé : suite `SkewRegimeOosIntegration` sur {QQQ_max, SKEW_max} — verrous axe, 3 pavages (750/500/500 canonique, 550/400/400 fin, offset 90 décalé), garde de trades D47, MC size-aware (seed 42, 2000 chemins), multi-univers QQQ/SPY/IWM/MDY, balayage thresholdMult × refLookback, verrou de fragilité (grille resserrée) si un candidat dSharpe > 0 émerge (leçon 8t.3). Critère : verrous verts, chiffres figés = chiffres MESURÉS (sentinelle → mesure → figé), suite complète verte.
  - [ ] **22.2** Verdict OOS VVIX verrouillé : suite `VvixRegimeOosIntegration`, même protocole que 22.1 (caveat consigné : axe ~2007+, pas de dot-com — même classe que D57).
  - [ ] **22.3** Section 22 du harnais `validate` : « INDICES DÉRIVÉS D'OPTIONS (SKEW / VVIX) », imprime les MÊMES chiffres que les verrous ; sections 1-21 inchangées.
  - [ ] **22.4** Clôture (`prompt-mise-a-jour-roadmap.md`) : cocher, changelog Sprint 22, découverte D61 (verdicts), rétrospective, recalage du décompte de tests, `documentation/CONCLUSION_RECHERCHE_EDGE.md` §5.4 annotée ; **retour au MODE MAINTENANCE** sauf si un candidat passe le gate complet de confirmation — l'adoption resterait une DÉCISION UTILISATEUR. Prod reste paper : la DoD d'edge n'est pas atteinte et le verrou `LiveTradingStaysDisapprovedUntilEdgeDoD` reste intact
...
- **État des tests** : 818/818 verts (636 unitaires + 182 intégration) — et la
  ...
  **--coverage**, et TSan ciblé **BLOQUANT** en CI sur les suites concurrentes
  (`WsServer|Watchdog|BotState`). +4 au Sprint 21 (suite `TempPathUnit` —
  contrat d'unicité des noms de fichiers temporaires, correctif D59 ;
  baseline recalée sur `ctest -N`=814 à l'ouverture, AUCUNE dérive hors
  cycle). Détail au changelog. Le caveat D59 du Sprint 20 (flake
