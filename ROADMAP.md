...
- **Dernière mise à jour** : 2026-07-09 (Sprint 20 — **Surface d'options ^SKEW/^VVIX**, décision utilisateur (r''). La piste §5.4 de `CONCLUSION_RECHERCHE_EDGE.md` (données alternatives / surface d'options) est ouverte dans sa variante GRATUITE (indices Yahoo dérivés de la surface d'options CBOE — décisions jointes : source à coût zéro, les DEUX signaux jugés) et SOLDÉE : **20.0** exports `SKEW_max.csv` (9117 barres 1990-01-02 → 2026-07-01, dot-com + 2008) et `VVIX_max.csv` (4895 barres 2007-01-03 → 2026-07-01), script `export_total_return.py` inchangé ; **20.1-20.2** les deux signaux jugés par le protocole complet (3 pavages, multi-univers, balayages, MC size-aware seed 42) via le moteur `VixRegimeBacktester` D51 RÉUTILISÉ tel quel (agnostique au signal — AUCUN code de prod nouveau, aucun test unitaire nouveau) → AUCUN EDGE : dSharpe OOS < 0 sur les 3 pavages, les 4 actifs et les 9 réglages pour les DEUX signaux (SKEW −0,68/−0,55/−0,67, le PIRE des filtres externes — 343 A/R, churn de coûts ; VVIX −0,61/−0,62/−0,66, même profil que le VIX D51), AUCUN candidat de balayage > 0 (pas de grille resserrée requise — contraste D57), alpha négatif et DD réduit < 50 % partout (D58) ; **20.3** section 22 du harnais. Direction canonique seule jugée (D43 — l'inversion consignée comme hypothèse distincte). Goldens byte-identiques, 814 → **828/828 verts**)
...
- **Sprint courant** : Sprint 21 — **Décision requise**. Le Sprint 20 (décision (r'')) a soldé la variante GRATUITE de la piste §5.4 (indices de surface d'options ^SKEW/^VVIX, D58) : 9 familles/variantes explorées depuis le Sprint 8, TOUTES sans edge — le périmètre à coût zéro (offline + indices gratuits) est désormais ÉPUISÉ. Options : (e) **mode maintenance** (recommandée — paper-only en veille : baseline verte au fil de l'eau, aucune exploration nouvelle) ; (a'') **finitions Production restantes** (clang-tidy ciblé, test de contrat cross-feed — reliquats D25/Sprint 17) ; ou (r''') **chantier PAYANT §5.4** (vraies chaînes d'options : IV par strike/échéance — nécessite une décision utilisateur sur la source payante (ORATS, CBOE DataShop…), son coût et la fourniture d'identifiants ; hors de portée sans cela). Prod reste paper — la DoD d'edge n'est pas atteinte et le verrou `LiveTradingStaysDisapprovedUntilEdgeDoD` reste intact
...
- **État des tests** : 828/828 verts (632 unitaires + 196 intégration) — et la
  ...
  **--coverage**, et TSan ciblé **BLOQUANT** en CI sur les suites concurrentes
  (`WsServer|Watchdog|BotState`). +14 au Sprint 20 (baseline recalée sur
  `ctest -N`=814 à l'ouverture, aucune dérive hors cycle : 0 unitaire —
  moteur réutilisé — + 14 verrous OOS pour les deux signaux de surface
  d'options). Détail au changelog.
...
| D58 | 🟡 | (Sprint 20) **Les indices de surface d'options (^SKEW, ^VVIX) n'ont AUCUN edge de régime — la variante GRATUITE de la piste §5.4 est soldée.** Gate binaire niveau-vs-médiane-glissante (moteur D51 `VixRegimeBacktester` RÉUTILISÉ tel quel : il est agnostique au signal, csvPaths = {actif, indice} — ZÉRO code de prod nouveau, zéro test unitaire nouveau, plus conforme à « moteurs séparés additifs » qu'un clone). **SKEW** (prix de la protection de queue, options OTM S&P 500 ; axe QQQ∩SKEW 6797 barres 1999-03-10 → 2026-07-01, dot-com + 2008 — même couverture que D51, PAS une première) : dSharpe OOS **−0,68/−0,55/−0,67** sur les 3 pavages — le PIRE des filtres de régime externes : le SKEW croise sa médiane bien plus souvent que le VIX (343 A/R vs 216) → churn de coûts sans pouvoir prédictif ; négatif sur les 4 actifs et les 9 réglages (meilleur rl=63/m=1,2 → −0,01 : frôle zéro SANS le franchir, aucun candidat → pas de grille resserrée, contraste D57) ; MC size-aware cagrP50 +1,64 %, ddP95 62,5 %. **VVIX** (vol-de-vol, options SUR le VIX ; axe 4895 barres 2007-01-03+, 2008 sans dot-com, comparable D57 ; axe identique pour les 4 actifs → 180 trades partout, gate pur signal) : même profil que le VIX D51 — Sharpe positif mais SOUS le B&H (dSharpe **−0,61/−0,62/−0,66**), négatif sur les 4 actifs et les 9 réglages (meilleur −0,25) ; MC cagrP50 +4,97 %, ddP95 35,7 %. La 2e dérivée (vol-de-vol) n'apporte RIEN de plus que la 1re (niveau du VIX). DD réduit mais < 50 % partout, alpha négatif partout. LIMITE consignée (D43) : seule la direction canonique du gate (long ssi signal ≤ mult × médiane) est jugée — l'inversion (« long quand SKEW > médiane ») est une hypothèse DISTINCTE non explorée, à ne rouvrir que sur décision utilisateur. | Consigné (verrous `SkewRegimeOosIntegration` ×7 / `VvixRegimeOosIntegration` ×7) ; variante GRATUITE de §5.4 SOLDÉE — il ne reste que le chantier PAYANT (vraies chaînes d'options), décision de source/coût requise |

## Changelog

### Sprint 20 — Surface d'options ^SKEW/^VVIX : la variante gratuite du §5.4 (2026-07-09)

**Contexte** : Sprint 20 marqué « Décision requise » à l'ouverture. Décisions
utilisateur (3) : (r'') ouvrir le chantier données alternatives §5.4 (contre
maintenance (e) et finitions (a'')) ; source = **indices GRATUITS Yahoo**
dérivés de la surface d'options CBOE (^SKEW 1990+, ^VVIX 2007+ — faisabilité
démontrée en session AVANT la décision : HTTP 200 via le proxy, granularité
quotidienne vérifiée), contre données payantes ORATS/CBOE ; périmètre =
**les DEUX signaux** jugés séparément. Protocole complet : 3 pavages
(canonique 750/500/500, fin 550/400/400, décalé offset 90), multi-univers,
balayages, grille resserrée conditionnelle (leçon 8t.3), MC size-aware
(seed 42, D45), gardes D33/D34/D35/D47, verdicts « sentinelle → mesure →
figée ». Livrable annoncé : un VERDICT verrouillé, pas une promesse de
rentabilité. Décision de conception clé : le moteur `VixRegimeBacktester`
(D51) est RÉUTILISÉ tel quel — il est agnostique au signal (csvPaths =
{actif tradé, indice signal}), donc AUCUN moteur nouveau, AUCUN test
unitaire nouveau (le moteur reste verrouillé par ses suites Sprint 14) :
plus conforme à « moteurs séparés additifs, goldens byte-identiques » qu'un
clone de ~320 lignes × 2.

**Baseline à l'ouverture** : **814/814 verte** (build −Werror sans warning,
Linux paquets système sans vcpkg, `ctest -N`=814, 632 unitaires + 182
intégration, aucune dérive — le tableau de bord était exact).

**Commits** (ordre chronologique = ordre d'exécution) :
- `8ef9284` feat(data) : export SKEW_max.csv + VVIX_max.csv (indices surface d'options ^SKEW/^VVIX — item 20.0)
- `ac5e351` test(skew-regime) : verdict OOS verrouillé du gate ^SKEW (item 20.1)
- `e5b90e3` test(vvix-regime) : verdict OOS verrouillé du gate ^VVIX (item 20.2)
- `9d35cf5` test(validation) : section 22 surface d'options ^SKEW/^VVIX + verdict figé (item 20.3)
- (clôture) docs : mise à jour roadmap + conclusion d'edge

**Items** :
- [x] **20.0** Exports `SKEW_max.csv` + `VVIX_max.csv` (porte de données) → `8ef9284`
  `scripts/export_total_return.py ^SKEW ^VVIX` SANS modification (leçon
  Sprint 14/19 : data gate FIRST). SKEW : 9117 barres 1990-01-02 →
  2026-07-01 ; VVIX : 4895 barres 2007-01-03 → 2026-07-01 (END_DATE figée).
  Gate qualité consigné : dates strictement croissantes, aucun close ≤ 0,
  Adj == Close partout (indices — ATTENDU, hors `auditTotalReturnCsv`),
  plages plausibles (SKEW 101-183, VVIX 60-208). Exceptions .gitignore
  miroir VIX/VIX3M + compile-defs dans les cibles validate ET
  integration_tests.
- [x] **20.1** Verdict OOS SKEW verrouillé → `ac5e351`
  7 verrous `SkewRegimeOosIntegration` (axe, 3 pavages, MC, multi-univers,
  balayage). Verdict D58 : AUCUN EDGE — le PIRE dSharpe des filtres externes
  (−0,68/−0,55/−0,67), churn de coûts (343 A/R vs 216 pour le VIX) ; aucun
  candidat de balayage > 0 → le verrou de fragilité conditionnel (pré-planifié,
  modèle Sprint 19) n'a PAS été nécessaire.
- [x] **20.2** Verdict OOS VVIX verrouillé → `e5b90e3`
  7 verrous `VvixRegimeOosIntegration`. Verdict D58 : AUCUN EDGE — même
  profil que le VIX D51 (dSharpe −0,61/−0,62/−0,66), la vol-de-vol n'apporte
  rien de plus que le niveau ; axe 2007+ identique pour les 4 actifs →
  180 trades partout (gate pur signal, constat consigné dans le verrou).
- [x] **20.3** Section 22 du harnais `validate` → `9d35cf5`
  Imprime les MÊMES chiffres que les verrous (par signal : 3 pavages, DD,
  multi-univers, balayage, MC) + verdict. Sections 1-21 vérifiées
  byte-identiques à la capture baseline.

**Verdicts (mesurés 2026-07-09, figés par test)** : voir D58. Aucun candidat
n'a émergé du balayage (contraste Sprint 19 : pas de grille resserrée à
dérouler) — le gate de confirmation est resté FERMÉ de bout en bout.
...
**Tests** : 814 → **828/828 verts** (+0 unitaire — moteur réutilisé —,
+14 intégration ; 632 + 196). Sections 1-21 du `validate` inchangées ;
section 22 ajoutée (verdicts imprimés = verdicts verrouillés).

**Rétrospective** :
1. *Découpage* : bon — la porte de données 20.0 EN PREMIER (leçon
   Sprint 14/19) a livré les comptes de barres réels avant tout verrou ;
   les deux verdicts se sont enchaînés sur le MÊME moteur sans surprise.
   La décision de RÉUTILISER le moteur D51 (au lieu de le cloner) a réduit
   le sprint à ~740 lignes de tests + données : c'est le premier sprint
   d'edge sans AUCUN code de prod nouveau — le protocole juge des DONNÉES,
   pas du code, et l'outillage existant a suffi.
2. *Prompts du workflow* : suffisants, AUCUN diff proposé. La contingence
   « candidat > 0 → grille resserrée » était pré-planifiée (leçon 8t.3/D57)
   mais n'a pas eu à servir ; la garde D47 non plus (les deux signaux
   tradent massivement). Le probing de faisabilité AVANT la décision
   utilisateur (HTTP 200, granularité quotidienne) a évité de choisir une
   option infaisable.
3. *À détecter plus tôt / garde-fou* : rien de nouveau — les caveats d'axe
   (SKEW = même couverture 1999+ que D51 ; VVIX = 2007+ sans dot-com) étaient
   anticipés par l'exploration et sont consignés dans D58 et les en-têtes
   des verrous ; les verrous `AxisIsLocked` figent les bornes (toute dérive
   de ré-export casse le test). Un point de vigilance consigné : le probe
   Yahoo `range=max` renvoie du MENSUEL (439 barres) là où la forme
   `period1=0` du script renvoie du quotidien — mesurer depuis le CSV réel,
   jamais depuis un probe, reste la règle (elle a été suivie).
4. *Notes /100* : **toutes inchangées** — Architecture 90, Qualité 94,
   FinTech 88, Production 78, **Rentabilité 28 (=)** : un verdict négatif de
   plus (le 9e) ne change ni la sûreté ni la capacité démontrée de gain ; la
   valeur du sprint est d'avoir soldé la variante GRATUITE du §5.4 en un
   sprint court (réutilisation de moteur) — le périmètre à coût zéro est
   désormais ÉPUISÉ, la suite est une décision de budget (chantier payant)
   ou de mise en veille.
