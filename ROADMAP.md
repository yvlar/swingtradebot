...
- **Dernière mise à jour** : 2026-07-14 (Sprint 23 — **Faisabilité d'un petit compte de 1 000 $**, ouvert et clos par décision utilisateur explicite (sortie TEMPORAIRE du mode maintenance, périmètre pré-décidé : simulation/paper-only, `config/prod.json` byte-identique, fractions STRICTEMENT offline). Baseline 832/832 verte (SHA `62aa240`). **23.0-23.1** scénarios 1 000 $ à risque 0,5/1/2 % + instrumentation des rejets par OBSERVATEURS opt-in (quantité zéro ≠ cash insuffisant ≠ risk-manager, aucun parsing de logs) ; **23.3** modèle de coûts configurable (%, par action, minimum/plafond par ordre, frais réglementaires — scénario A reproduit le golden AU CENTIME) ; **23.4** B&H NET (coûts 2 côtés + actions entières + cash résiduel : +207,69 % net vs +226,12 % brut) ; **23.2/23.5** rapport de faisabilité à verdict déterministe + contre-factuel fractionnaire analytique ; **23.6** stress séquences de pertes composées/gaps/slippage ; **23.7** Monte-Carlo par BLOCS (IID historique intact) ; **23.8** verdicts verrouillés (8 verrous `SmallAccountFeasibilityIntegration`) : risque 0,5 % INEXPLOITABLE (0/861 entrées), 1 % FRACTIONS_POTENTIELLEMENT_NECESSAIRES (3/764, 99,6 % bloquées), 2 % EXPLOITABLE_ACTIONS_ENTIERES (22/22) mais alpha net −192 pt — ET au dernier close QQQ (~600 $) AUCUN risque testé ne peut ouvrir une position (D62). AUCUN EDGE DÉMONTRÉ — SwingBot RESTE PAPER-ONLY. 832 → **899/899 verts**. Goldens byte-identiques, `config/prod.json` intact, verrou live intact, brokers IBKR/Alpaca inchangés (quantités entières). RETOUR AUTOMATIQUE au mode maintenance)
- **Mise à jour précédente** : 2026-07-11 (Sprint 22 — **Chantier (r'') : indices dérivés d'options SKEW/VVIX**, ouvert par décision utilisateur explicite = SORTIE du mode maintenance (RUNBOOK §8.5). **22.0** porte de données SKEW/VVIX ; **22.1/22.2** verdicts verrouillés : AUCUN EDGE (D61, aucun candidat dSharpe > 0) ; **22.3** section 22 du `validate` ; **22.4** clôture. 818 → 832/832 verts. La famille « niveau d'indice vs sa médiane » est soldée sur 4 signaux ; le volet GRATUIT du §5.4 est épuisé)
- **Sprint courant** : — **MODE MAINTENANCE ACTIF (retour AUTOMATIQUE à la clôture du Sprint 23, 2026-07-14)** — conformément à la décision d'ouverture du Sprint 23 (« sortie temporaire du mode maintenance uniquement pour le périmètre défini »), SwingBot RETOURNE au mode maintenance décidé au Sprint 20 (décision (e)) : banc d'essai paper-only en veille, régi par `RUNBOOK.md` §8 (baseline verte au fil de l'eau ; toute modification exige une preuve objective, reproductible et documentée — test rouge lorsqu'applicable — et un item de sprint dédié). Le mode maintenance est un ÉTAT TERMINAL TEMPORAIRE, pas un sprint vide : tant qu'il est actif et qu'aucun nouveau sprint n'a été défini par décision utilisateur, `prompt-executer-sprint.md` ne doit PAS être lancé. Réouverture UNIQUEMENT par décision utilisateur explicite (précédents : Sprints 22 et 23). Options connues au backlog : (a'') finitions Production (clang-tidy ciblé, contrat cross-feed — reliquats D25/Sprint 17, constats D58), (r''') données alternatives PAYANTES ou d'une autre classe (le volet gratuit du §5.4 est ÉPUISÉ, D61), ou (s) support des fractions d'action dans la simulation/les brokers (D62 — refonte des types de quantité int, décisions utilisateur préalables requises). Prod reste paper — la DoD d'edge n'est pas atteinte (confirmé une 11e fois par le Sprint 23 : alpha net −192 pt) et le verrou `LiveTradingStaysDisapprovedUntilEdgeDoD` reste intact
...
- **État des tests** : 832/832 verts (636 unitaires + 196 intégration) — et la
  ...
  **--coverage**, et TSan ciblé **BLOQUANT** en CI sur les suites concurrentes
  (`WsServer|Watchdog|BotState`). +14 au Sprint 22 (verrous
  `SkewRegimeOosIntegration` ×7 + `VvixRegimeOosIntegration` ×7 — verdicts
  D61 ; baseline recalée sur `ctest -N`=818 à l'ouverture, AUCUNE dérive
  hors cycle). Détail au changelog. Le caveat D59 du Sprint 20 (flake
  d'isolation DbLogger en CI) est ENTIÈREMENT LEVÉ : la CI complète du SHA
  final `374f64c` (run 29134734275) est TERMINÉE VERTE le 2026-07-11
  (règle §8.4 satisfaite — constaté à l'ouverture du Sprint 22).
...
| D18 | 🟡 | (Sprint 3) L'ATR de l'item 13 est une **approximation clôture-à-clôture** : `IIndicator<double>` ne reçoit que la série des clôtures, pas les high/low — le vrai true range est inaccessible via cette interface. Décision produit : enrichir l'interface (compute sur `vector<Bar>`) ou assumer l'approximation (documentée dans DayIndicators.hpp) | ✅ Déjà résolu en code (constaté Sprint 15, D52) : la voie true-range `ATR::computeBars(vector<Bar>)` existe (`DayIndicators.hpp:56-86`), le seam `IIndicator::computeBars` aussi (`Interfaces.hpp:92-97`), et c'est la SEULE voie ATR utilisée en prod (`RiskManager.hpp:65,198`). La voie close-à-close `compute` est du code mort gardé pour le contrat pur-virtuel. Aucun correctif nécessaire ; `DayTradeStrategy` reste dormante (suppression = décision utilisateur) |
| D19 | 🟢 | (Sprint 3) `TradingBot::runOnce` code en dur `getBars(symbol, 60)` (TradingBot.hpp:62) alors que le backtest sert une fenêtre de emaSlow+30=51 barres via ReplayDataFeed — la taille de fenêtre influence le seed SMA des EMA, donc les signaux. Bénin tant que les feeds prod renvoient ≥51 barres, mais un `lookback` configurable (RiskConfig ?) unifierait prod et backtest | **Re-priorisé Sprint 9 (9.2)** : l'item 20 du Sprint 5 s'est limité au calendrier/DST (aucun changement de fenêtre de données) ; le lookback unifié appartient à la mise en prod de la stratégie validée (E5), pas au durcissement runtime ; **constaté résolu en pratique au Sprint 15 (D52)** : `TradingBot.hpp:88` lit déjà `riskCfg_.lookback` (plus de `60` en dur), aligné par la conversion SwingStrategy sur `max(60, smaTrendPeriod+30)`=230 = la formule du backtest → prod/backtest coïncident pour la config prod (résidu : cas régime désactivé) |
...
| D25 | 🟡 | (Méta-audit) Prod : boucle 60 min sur barres **journalières** → la dernière barre n'est pas clôturée, le croisement EMA peut osciller intra-journée (flap/look-ahead absent du backtest qui ne voit que des barres complètes) | ✅ Déjà mitigé en code (constaté Sprint 15, D52) : la barre du jour EN FORMATION est droppée dans `IBKRDataFeed.hpp:100-101` (verrouillé par test) → chaque cycle ne voit que des barres CLÔTURÉES, le croisement ne peut plus osciller ; `runOnce` idempotent sur la même barre (gardes `lastBarDate`/`lastExitDate`/`stopArmed`). Résidu backlog : test de contrat inter-feed |
...
| D54 | 🟢 | (Sprint 17) **La liste apt de `prompt-executer-sprint.md` (fichier GOUVERNÉ) ne contient pas `libssl-dev`, désormais requis** : l'item 17.1 lie OpenSSL aux tests d'intégration (`MiniSmtpServer`, mock STARTTLS). La CI (4 jobs) et les Dockerfiles l'installent déjà ; seul le prompt gouverné est en retard — un exécuteur de sprint sur Linux nu partirait d'une baseline rouge (échec `find_package(OpenSSL)`). Conformément à la règle « les fichiers de règles ne sont JAMAIS modifiés sans décision utilisateur », le diff est PROPOSÉ, pas appliqué : ajouter `libssl-dev` entre `libsqlite3-dev` et `libgtest-dev` dans la commande apt (ligne ~20). En attendant, `find_package(OpenSSL REQUIRED)` échoue avec un message explicite — le manque se diagnostique en une ligne | ✅ APPLIQUÉ au Sprint 19 (`6ece6d9`) — décision utilisateur explicite à l'ouverture du sprint, commit dédié citant la décision |
| D55 | 🟡 | (Sprint 18) **Le scaling continu Moreira-Muir COMBLE quasiment l'écart de Sharpe vs B&H — le résultat négatif LE PLUS SERRÉ du projet — mais l'alpha absolu reste négatif et le signe du dSharpe est instable.** Le poids continu w = min(1, cible/vol annualisée) avec bande anti-churn 0,05 et coûts ∝ |Δw| (`VolScaledBacktester`) donne dSharpe OOS +0,06 (canon) / −0,06 (fin) / +0,03 (décalé) là où le filtre BINAIRE long/cash plafonnait à −0,5 (D50) : l'hypothèse « le cash drag est le vrai coût du binaire » est CONFIRMÉE — rester partiellement investi en vol haute récupère l'essentiel du Sharpe perdu. MAIS : (1) l'alpha absolu vs B&H reste négatif sur les 3 pavages (−4,5/−3,3/−8,1) et les 4 actifs (SPY −3,7 / IWM −7,4 / MDY −5,4) ; (2) le meilleur réglage du balayage {10,15,20}×{0,02;0,05;0,10} frôle zéro par en-dessous (cible 20/bande 0,10 → −0,52) sans le franchir ; (3) le signe du dSharpe s'inverse selon le pavage (leçon 8t.1 : pas un edge) ; (4) le DD n'est réduit que d'~1/3 (15,2 vs 23,5 %) — la clause de repli exige ≥ 50 %. La variante à vol IMPLICITE (`VixScaledBacktester`, w = cible/VIX, décidable dès la barre 0) fait MOINS bien que la réalisée (alpha −10,9/−8,6/−11,8, dSharpe < 0 partout) — cohérent avec D51. MC size-aware cagrP50 +11,2 % (capte une bonne part de la hausse, moins que le B&H). | Consigné (verrous `VolScaledOosIntegration` / `VixScaledOosIntegration`) ; piste §5.2 SOLDÉE — un franchissement exigerait du levier (maxWeight > 1, hors périmètre risque) ou une cible/formule différente = D43 s'appliquerait |
...
| D62 | 🟡 | (Sprint 23) **La contrainte d'action entière DOMINE le compte de 1 000 $ — et la faisabilité observée à risque 2 % est un artefact de prix historique.** Mesures verrouillées (`SmallAccountFeasibilityIntegration`, QQQ.csv 2019-2026, coûts scénario A) : à risque 0,5 % le compte ne peut JAMAIS trader (861/861 rejets quantité zéro — budget de risque 5 $ contre ≥ 7 $ de risque par action) ; à 1 % il exécute 3 entrées sur 764 tentatives (99,6 % bloquées) ; à 2 % il exécute 22/22 MAIS uniquement parce que l'historique 2019-2021 cotait 150-400 $ : au dernier close de la série (600,50 $), AUCUN des trois risques ne peut ouvrir une position (2 % → budget 20 $ < 30 $ de risque par action ; la séquence de stress est bloquée 10/10). Le contre-factuel fractionnaire analytique (23.5, STRICTEMENT offline) chiffre la perte : 11,3 points de déploiement perdus par la troncature même dans le scénario exécutable, 761-861 signaux bloqués UNIQUEMENT par la contrainte entière aux risques prudents. Conclusion : opérer 1 000 $ au prix ACTUEL du QQQ exigerait le support des fractions d'action — soit une simulation fractionnaire complète, soit les brokers (IBKR/Alpaca les offrent en réel). Les deux exigent une refonte des types de quantité `int` de `IBroker::submitBuy/submitSell` et du sizing : chantier HORS du périmètre 23 (décision d'ouverture : aucune fraction côté live), à ouvrir par décision utilisateur | Consigné (backlog option (s)) — le rapport 23 conclut « fractions potentiellement nécessaires » ; aucun sprint affecté tant que le mode maintenance dure |
| D63 | 🟢 | (Sprint 23) **Le bootstrap par blocs alourdit la queue de drawdown vs l'IID sur les trades du petit compte — les grappes de pertes comptent.** Sur les 22 trades du scénario 1 000 $/risque 2 % (7,12 ans, graine 42, 2 000 chemins) : DD p95 IID 5,27 % contre 6,68 % en blocs de 3 (+27 % relatif) — l'IID, en détruisant l'autocorrélation, sous-estime le risque de queue quand les pertes arrivent groupées (régimes) ; en blocs de 10, 4,39 % (2 blocs par chemin → diversité réduite, le chemin ressemble à l'observé). Le MC IID historique reste INCHANGÉ (goldens verts) — la variante blocs (`BlockBootstrap.hpp`, ordre interne conservé, longueur de chemin = n trades) est un COMPLÉMENT validé sur séquence synthétique en grappes avant lecture des vrais chiffres (leçon D45 appliquée avant coup). Caveat : 22 trades est une petite base — la fourchette 4,4-6,7 % est indicative, pas prédictive | Consigné (verrou `MonteCarloIidVsBlocksIsLocked`) — pour les futurs verdicts de queue, lire IID ET blocs |

## Changelog

### Sprint 23 — Faisabilité d'un petit compte de 1 000 $ (2026-07-14)
...
- `be99ef9` docs : ouverture Sprint 23 — faisabilité compte 1000 $ (décision utilisateur, sortie temporaire du mode maintenance)
- `c3eb52a` feat(backtest) : modèle de coûts configurable petit compte (item 23.3)
- `7ef7abd` feat(backtest) : scénarios de capital et instrumentation des rejets (items 23.0-23.1)
- `2d734a9` feat(backtest) : benchmark buy-and-hold net de coûts (item 23.4)
- `7dc3e90` feat(backtest) : rapport de faisabilité 1000 $ + analyse offline des fractions (items 23.2, 23.5)
- `0dad4ce` feat(backtest) : stress petit compte — séquences de pertes et gaps (item 23.6)
- `d81ed84` feat(backtest) : bootstrap Monte-Carlo par blocs (item 23.7)
- `ab66961` test(backtest) : section 23 du validate + verrous intégration faisabilité compte 1000 $ (items 23.2, 23.8)
- (clôture) docs : verdict final, D62/D63, rétrospective, retour au mode maintenance

**Items** (dépendances : 23.1→23.0 ; 23.2→23.0/23.1/23.3/23.4 ; 23.4→23.3 ;
23.5→23.1 ; 23.6→23.3 ; 23.8→tous) :
- [x] **23.0** Scénarios de capital et de risque → `7ef7abd`
  Capital initial 1 000 $ réellement injecté dans le backtester
  (`include/backtest/BackTester.hpp` accepte déjà `initialCapital` ;
  `include/bot/RiskManager.hpp` dimensionne depuis le cash) ; risques
  0,5 %/1 %/2 % fournis par `SmallAccountScenario` (structure offline),
  JAMAIS par `config/prod.json`. Acceptation SATISFAITE : les trois niveaux
  tournent au harnais ; l'ancien comportement 10 000 $ intact (le scénario A
  redonne le golden 11 932,57 $ AU CENTIME) ; goldens inchangés ; le verrou
  `InitialCapitalDrivesSharesAndReturn` démontre l'influence du capital
  (rendement 15,56 % à 1 000 $ contre 19,33 % à 10 000 $, taille moyenne
  1,18 contre 16,09 actions).
- [x] **23.1** Instrumentation des signaux non exécutables → `7ef7abd`
  Compteurs
...
- [x] **23.2** Rapport `SmallAccountFeasibilityReport` → `7dc3e90` + `ab66961`
...
- [x] **23.3** Modèle de coûts configurable → `c3eb52a`
  Abstraction {`commissionPct`,
...
- [x] **23.4** Benchmark brut et net → `2d734a9`
  `buyHoldGrossReturnPct` (métrique
...
- [x] **23.5** Analyse des fractions d'action STRICTEMENT offline → `7dc3e90`
  Aucun type
...
- [x] **23.6** Tests de survie du petit compte → `0dad4ce`
  Séquences de 5 et 10 pertes au
...
- [x] **23.7** Monte-Carlo par blocs → `d81ed84`
  Bootstrap par BLOCS de transactions
...
- [x] **23.8** Verdict final → `ab66961` + (ce commit)
  Réponses explicites aux 10 questions du sprint
...
Toutes RESPECTÉES à la clôture (vérifié : `git diff` de `config/prod.json`
vide, verrou live vert, prompts intacts, `submitBuy/submitSell(int)` inchangés).

**Verdicts (mesurés 2026-07-14, figés par les 8 verrous
`SmallAccountFeasibilityIntegration`)** — QQQ.csv total-return 2019-2026,
config prod, coûts scénario A (0,1 %/côté + 2,5 bps), capital 1 000 $ :
- **Risque 0,5 %** : `INEXPLOITABLE_ACTIONS_ENTIERES` — 861 signaux BUY, 861
  rejets quantité zéro, 0 entrée, capital intact (budget de risque 5 $ contre
  ≥ 7 $ de risque par action sur toute la série).
- **Risque 1 %** : `FRACTIONS_POTENTIELLEMENT_NECESSAIRES` — 3 entrées sur 764
  tentatives (99,6 % bloquées par la contrainte entière), final 1 040,76 $.
- **Risque 2 %** : `EXPLOITABLE_ACTIONS_ENTIERES` — 22/22 entrées (les mêmes
  signaux que le golden), final 1 155,60 $ (+15,56 %), frais 14,48 $ (1,45 %
  du capital ; 44 $ soit 4,4 % au stress C — un minimum par ordre DOMINE),
  déploiement moyen 28,7 % contre 40 % théorique fractionnaire (11,3 points
  perdus par la troncature), DD max 5,28 %. MAIS : faisabilité ≠ rentabilité —
  alpha net **−192,13 pt** vs B&H net (+15,56 % contre +207,69 %).
- **B&H 1 000 $** : brut +226,12 % / net +207,69 % (coûts 2 côtés + actions
  entières + cash résiduel — l'écart de 18 pts est le « coût » réel du B&H).
- **Le constat qui gouverne tout (D62)** : la faisabilité du risque 2 % est un
  ARTEFACT DE PRIX — l'historique 2019-2021 cotait 150-400 $ ; au dernier
  close (600,50 $), AUCUN risque testé ne peut ouvrir une position (2 % →
  budget 20 $ < 30 $ de risque par action). Un compte de 1 000 $ qui
  démarrerait AUJOURD'HUI ne peut pas exécuter la stratégie en actions
  entières, à aucun niveau de risque prudent.
- **Stress (scénario A, prix 100 $)** : 5 pertes → −8,35 %, 10 pertes →
  −16,18 % (composition réelle, SOUS le naïf 20 % — le re-sizing entier
  amortit) ; gap −10 % au lieu du stop −5 % → perte 40,86 $ = 2× la perte
  planifiée (20 $) ; slippage 2→25 bps → −31 $ de capital final.
- **Monte-Carlo (22 trades, 7,12 ans, graine 42, 2 000 chemins)** : IID DD p95
  5,27 % ; blocs de 3 : 6,68 % (les grappes de pertes ALOURDISSENT la queue,
  +27 % relatif) ; blocs de 10 : 4,39 %. MC IID historique INCHANGÉ (goldens
  verts). Caveat : 22 trades est une petite base — ces DD sont indicatifs.
- **Verdict final (23.8)** : AUCUN EDGE DÉMONTRÉ. LE CAPITAL DE 1 000 $ NE
  JUSTIFIE PAS UNE ACTIVATION DU TRADING RÉEL. SWINGBOT RESTE PAPER-ONLY.

**Tests** : 832 → **899/899 verts** (+59 unitaires : ExecutionCostModelUnit 15,
ExecutionStatsUnit 9, NetBuyHoldUnit 7, FractionalSizingUnit 5,
SmallAccountFeasibilityUnit 7, SmallAccountStressUnit 9, BlockBootstrapUnit 7 ;
+8 intégration : SmallAccountFeasibilityIntegration). DoD vérifiée localement :
Debug ET Release 899/899, ASan+UBSan suite complète 899/899, TSan ciblé
(WsServer|Watchdog|BotState) 75/75, cppcheck vert (commande CI exacte),
couverture gcovr 97,6 % de lignes sur include/ (seuil 95 % — en hausse vs
97,2 % à la pose du gate). Sections 1-22 du `validate` inchangées.

**Rétrospective** :
1. *Découpage* : bon — le modèle de coûts (23.3) AVANT l'instrumentation a
   évité de re-toucher deux fois le PaperBroker ; chaque item = un commit
   atomique avec ses tests ; la préservation des goldens a été PROUVÉE au fil
   de l'eau (le scénario A redonne 11 932,570159 $ au micro-centime via le
   chemin neuf — meilleure preuve possible que l'ancien chemin est intact).
2. *Prompts du workflow* : suffisants, AUCUN diff proposé. Le prompt du sprint
   imposait des seuils de verdict « documentés » sans les fixer : ils ont été
   choisis (blocage ≥ 50 % → fractions ; tout blocage ou ≥ 15 pts de
   déploiement perdus → contraint) et consignés dans le code
   (`SmallAccountFeasibility.hpp`) ET ici — re-calibrables par simple décision
   utilisateur, pas de re-baseline cachée.
3. *À détecter plus tôt / garde-fous* : (a) le P&L des TradeRecords du chemin
   « modèle de coûts » inclut la commission d'ACHAT (sinon un minimum par
   ordre serait invisible au rapport), là où le chemin historique ne compte
   que la vente (convention D22 des goldens) — divergence DOCUMENTÉE dans le
   code, verrouillée par test, à unifier seulement si un sprint futur
   re-baseline les goldens ; (b) la classification quantité-zéro/cash repose
   sur le sizing historique — le vol-sizing 8o.2 (désactivé en prod) peut
   sous-classer, noté dans TradingBot.hpp ; (c) leçon D45 réappliquée AVANT
   coup cette fois : le MC par blocs a été validé sur séquence synthétique en
   grappes (le test prouve qu'il mesure bien ce qu'il prétend) avant de lire
   ses chiffres sur les vrais trades.
4. *Notes /100* : **toutes inchangées** — Architecture 90 (=) : seams
   d'observation opt-in et scénarios de coûts sont des AJOUTS propres,
   aucune interface live touchée ; Qualité 94 (=) : +67 tests, mais la
   méthodologie (rouge→vert, verrous, calculs manuels) est celle déjà notée ;
   FinTech 88 (=) : le réalisme des coûts progresse (minimum par ordre,
   B&H net) mais aucun mécanisme de trading nouveau n'est validé ;
   Production 78 (=) : rien de déployable n'a changé. **Rentabilité 28 (=)**
   par construction : le sprint MESURE la faisabilité, il ne cherche pas
   d'edge — l'alpha net −192 pt confirme une 11e fois le verdict paper-only ;
   la note ne bougera qu'avec un edge absolu net de coûts (DoD inchangée).

### Sprint 22 — Chantier (r'') : indices dérivés d'options SKEW/VVIX (2026-07-11)

**Contexte** : décision utilisateur explicite à l'ouverture = SORTIR du mode
maintenance (RUNBOOK §8.5, conditions 1-6 satisfaites : décision consignée,
sprint défini avec items numérotés et critères d'acceptation, dépendances
rendues à l'ouverture) en rouvrant le chantier (r'') « données alternatives /
surface d'options » (§5.4, la SEULE piste restante de la conclusion d'edge).
Périmètre décidé par l'utilisateur : données = indices CBOE dérivés de la
surface d'options `^SKEW` (prix du risque de queue, 1990+) et `^VVIX` (vol de
la vol, ~2007+) ; fournisseur/budget/licence = pipeline Yahoo v8 existant
(script INCHANGÉ, coût 0, même canal que ^VIX/^VIX3M) ; hypothèses jugées
SÉPARÉMENT = (a) régime SKEW (queue chère → cash), (b) régime VVIX ;
benchmark et DoD INCHANGÉS. Protocole complet : 3 pavages, multi-univers,
balayages, grille resserrée si candidat (leçon 8t.3), MC size-aware (seed 42,
D45), gardes D33/D34/D35/D47, verdicts « sentinelle → mesure → figée ».
Moteur `VixRegimeBacktester`/`VixRegimeWalkForward` (Sprint 14) réutilisé TEL
QUEL — aucun nouveau moteur, aucun nouveau test unitaire requis.

**Baseline à l'ouverture** : **818/818 verte** (build −Werror sans warning,
Linux paquets système sans vcpkg, `ctest -N`=818 = tableau de bord, AUCUNE
dérive) ET la CI complète du SHA de tête `374f64c` (run `29134734275`)
constatée TERMINÉE VERTE → le caveat §8.4 du Sprint 21 est LEVÉ : la clôture
du Sprint 21 est entièrement validée.

**Commits** (ordre chronologique = ordre d'exécution) :
- `66be21a` docs(roadmap) : ouverture du Sprint 22 — chantier (r'') indices dérivés d'options SKEW/VVIX (décision utilisateur, sortie du mode maintenance §8.5 ; CI 374f64c verte, caveat §8.4 levé)
- `8e03a1a` feat(data) : export SKEW_max.csv + VVIX_max.csv (indices ^SKEW/^VVIX, chantier (r'') — item 22.0)
- `b8fca82` test(skew-regime) : verdict OOS verrouillé du filtre SKEW — aucun edge (item 22.1)
- `7411ec4` test(vvix-regime) : verdict OOS verrouillé du filtre VVIX — aucun edge (item 22.2)
- `4d5c540` test(validation) : section 22 indices dérivés d'options SKEW/VVIX + verdict figé (item 22.3)
- (clôture) docs : mise à jour roadmap + conclusion d'edge

**Items** :
- [x] **22.0** Porte de données SKEW/VVIX → `8e03a1a`
  `scripts/export_total_return.py ^SKEW ^VVIX` SANS modification (caret déjà
  encodé, fallback adjclose→close déjà en place) : 9117 barres 1990-01-02 →
  2026-07-01 (SKEW) et 4895 barres 2007-01-03 → 2026-07-01 (VVIX), END_DATE
  figée. Gate qualité : dates strictement croissantes, aucun close ≤ 0,
  Adj == Close partout (indices — ATTENDU, hors `auditTotalReturnCsv`).
  Exceptions .gitignore miroir des Sprints 14/19.
- [x] **22.1** Verdict OOS SKEW verrouillé → `b8fca82`
  7 verrous `SkewRegimeOosIntegration` (axe 6797 barres 1999+ dot-com/2008
  couverts, 3 pavages, MC size-aware, multi-univers, balayage 12 réglages —
  plage resserrée {0,95-1,1}, l'indice étant borné). Verdict D61 : AUCUN
  EDGE — dSharpe −0,68/−0,55/−0,67, meilleur réglage −0,08, aucun candidat
  → pas de grille resserrée (sans objet).
- [x] **22.2** Verdict OOS VVIX verrouillé → `7411ec4`
  7 verrous `VvixRegimeOosIntegration` (axe 4895 barres 2007+ — caveat
  classe D57 : pas de dot-com ; balayage 9 réglages vol-like {0,8-1,2}).
  Verdict D61 : AUCUN EDGE — dSharpe −0,61/−0,62/−0,66, meilleur réglage
  −0,25, aucun candidat.
- [x] **22.3** Section 22 du harnais `validate` → `4d5c540`
  Imprime les MÊMES chiffres que les verrous (2 indices × [3 pavages +
  multi-univers + balayage + MC]). Sections 1-21 vérifiées BYTE-IDENTIQUES
  (diff avant/après sur la sortie complète).
- [x] **22.4** Clôture → (ce commit)
  Changelog, D61, rétrospective, recalage des tests, §5.4 de
  `CONCLUSION_RECHERCHE_EDGE.md` annotée, retour au MODE MAINTENANCE.

**Verdicts (mesurés 2026-07-11, figés par test)** : voir D61. Aucun candidat
dSharpe > 0 n'a émergé d'aucun des deux balayages — le gate de confirmation
(D43) n'a même pas été approché. `config/prod.json` intact,
`liveTradingApproved` = false, verrou `LiveTradingStaysDisapprovedUntilEdgeDoD`
intact, goldens byte-identiques.

**Tests** : 818 → **832/832 verts** (+14 intégration ; 636 unitaires + 196
intégration). Sections 1-21 du `validate` inchangées ; section 22 ajoutée
(verdicts imprimés = verdicts verrouillés).

**Rétrospective** :
1. *Découpage* : bon — la porte de données 22.0 EN PREMIER (leçon Sprints
   14/19) a levé la seule incertitude bloquante (fetch ^SKEW/^VVIX : HTTP 200
   via le proxy de session) avant tout code ; la réutilisation TELLE QUELLE du
   moteur générique du Sprint 14 a réduit le sprint à « données + verrous +
   CLI » (aucun moteur, aucun test unitaire nouveau — c'est la première fois
   qu'une famille est jugée sans écrire une ligne de code moteur).
2. *Prompts du workflow* : suffisants, AUCUN diff proposé. La seule
   adaptation (plage de balayage resserrée pour le SKEW, indice borné
   ~100-183 là où les seuils VIX {0,8;1,2} auraient donné toujours-cash /
   toujours-long) est une décision d'ANALYSE documentée dans l'en-tête du
   test, pas une improvisation de processus.
3. *À détecter plus tôt / garde-fou* : le caveat d'axe du VVIX (2007+, pas
   de dot-com) était anticipé dès l'ouverture (classe D57 déjà connue) et
   consigné dans l'en-tête du verrou + D61 ; le verrou `AxisIsLocked` fige
   les bornes comme au Sprint 19. Rien de nouveau à ajouter au backlog — le
   garde-fou existant a suffi.
4. *Notes /100* : **toutes inchangées** — Architecture 90, Qualité 94,
   FinTech 88, Production 78, **Rentabilité 28 (=)** : deux verdicts négatifs
   de plus (9e et 10e familles/variantes) ne changent ni la sûreté ni la
   capacité démontrée de gain ; la valeur du sprint est d'avoir soldé le
   volet GRATUIT du §5.4 proprement (la famille « niveau vs médiane » est
   morte sur 4 signaux) et d'avoir borné ce qui reste : des données PAYANTES
   ou une autre classe de données alternatives.

### Sprint 21 — Maintenance technique : flake DbLogger (D59) + diff CLAUDE.md (D60) (2026-07-10)

**Contexte** : premier sprint exécuté SOUS le mode maintenance (RUNBOOK §8.2 —
« cycle normal » sur preuve objective), sans en sortir. Ouverture par décision
utilisateur explicite : demande d'exécution de `prompt-executer-sprint.md` +
approbation du diff D60 (question posée et répondue « Oui, appliquer D60 »
avant tout code). Périmètre : les deux seuls reliquats actionnables du
Sprint 20 — D59 (flake d'isolation DbLogger, preuve objective = journaux CI
du run `29094492121`, item désigné « candidat du prochain sprint de
maintenance ») et D60 (diff `CLAUDE.md` en attente de décision). Les options
(a'')/(r'') restent fermées (décisions préalables non rendues).

**Baseline à l'ouverture** : **814/814 verte** (Linux paquets système sans
vcpkg, liste apt du prompt gouverné, build Debug `-Werror` sans warning,
`ctest -N` = 814 = tableau de bord — AUCUNE dérive hors cycle, suite complète
verte en 32,6 s).

**Commits** (ordre chronologique = ordre d'exécution) :
- `617221f` docs(regles) : applique le diff D60 — TSan bloquant et six jobs CI dans CLAUDE.md (décision utilisateur, Sprint 21)
- `893db4d` fix(tests) : noms de fichiers SQLite uniques par PID+compteur — flake d'isolation D59 (item 21.1)
- `d361667` refactor(tests) : généralise les noms temporaires uniques aux fixtures CSV/JSON/log (item 21.1, suite D59)
- (clôture) docs : clôture sprint 21 — mise à jour roadmap

**Items** :
- [x] **21.0 (gouverné)** Diff D60 appliqué à `CLAUDE.md` ligne 50 → `617221f`
  Décision utilisateur explicite à l'ouverture ; commit dédié citant la
  décision (règle « pas d'auto-amendement » respectée — précédent D54).
  La phrase CI dit désormais : six jobs, tous bloquants, TSan ciblé
  bloquant, lint cppcheck et couverture gcovr énumérés.
- [x] **21.1** Flake d'isolation D59 corrigé → `893db4d` + `d361667`
  Helper partagé `tests/support/TempPath.hpp` :
  `uniqueTempName(prefix, ext)` = `<prefix><PID>_<compteur atomique>_<tick><ext>`
  — le PID sépare deux processus ctest parallèles nés dans le même tick
  (la cause racine), le compteur les appels d'un même processus, le tick
  évite la réutilisation entre runs. Seam `uniqueTempNameForTick` (tick
  paramétré) pour tester le scénario de collision de façon déterministe.
  **Test rouge d'abord** (nouvelle suite `TempPathUnit`, 4 tests) : sur la
  transposition fidèle de l'ancien schéma (nom dérivé du seul tick),
  `SameTickTwiceStillYieldsDistinctNames` et `NameEmbedsProcessId`
  échouaient ; verts après le correctif. Migration en deux temps :
  (1) les 3 fixtures SQLite où la collision se manifeste en « database is
  locked » (test_db_logger_unit, test_state_store_unit,
  test_db_logger_integration) ; (2) généralisation par AUDIT (grep
  `time_since_epoch`) aux 7 fixtures CSV/JSON/log portant le même motif
  (csv_data_feed, config_loader, logger, data_quality, walk_forward,
  backtester_metrics, backtester_integration, multi_asset_integration) —
  même défaut, symptôme différent (écrasement de fichier au lieu du verrou
  SQLite). Plus AUCUNE occurrence du motif dans `tests/*.cpp`.
  Stress local : `ctest -R 'DbLogger|StateStore' --repeat until-fail:30
  -j16` → 100 % vert (la collision n'était de toute façon pas reproduite
  localement — la preuve du défaut reste les journaux CI, D59).

**Verdicts** : aucun verdict d'edge (sprint technique). `config/prod.json`
intact, `liveTradingApproved` = false, verrou
`LiveTradingStaysDisapprovedUntilEdgeDoD` intact, goldens byte-identiques
(aucun fichier sous `include/` touché — seul `tests/` et la liste des
sources de test du CMakeLists ont bougé).

**Tests** : 814 → **818/818 verts localement** (636 unitaires + 182
intégration ; +4 = suite `TempPathUnit`). Décompte d'ouverture recalé sur
`ctest -N` = 814 : aucune dérive hors cycle.

**Validation CI du SHA final** : la CI est déclenchée par le push de la
branche ; conformément à la règle §8.4, AUCUNE fusion ni déclaration
« verte » tant que les six jobs du SHA final ne sont pas terminés et verts.
Un passage vert du job Debug validera aussi rétroactivement le reliquat du
Sprint 20 (dont la CI était rouge sur le flake D59, désormais corrigé).
L'état constaté au moment de la clôture est consigné dans la conversation
de session ; le résultat définitif se lit sur le run GitHub Actions du SHA.

**Rétrospective** :
1. *Découpage* : adapté — 2 items indépendants (un documentaire gouverné,
   un technique), commits atomiques ; la généralisation 21.1-(2) découle
   d'un audit fait pendant le sprint plutôt que d'un item planifié, mais
   reste dans le périmètre D59 (même motif, même preuve par inspection).
2. *Prompts du workflow* : suffisants pour un sprint de maintenance —
   la combinaison RUNBOOK §8.2 (preuve objective) + « Décision requise »
   a fonctionné : l'exécution a été BLOQUÉE jusqu'aux décisions
   utilisateur (périmètre + D60), posées explicitement avant tout code.
   Aucun amendement de prompt nécessaire ni proposé.
3. *À détecter plus tôt / garde-fou* : le motif tick-seul existait depuis
   le Sprint 1 dans 10 fichiers — un grep `time_since_epoch` sur
   `tests/*.cpp` l'aurait attrapé dès la première collision. Garde-fou
   adopté SANS coût : le contrat d'unicité est désormais centralisé dans
   UN helper testé (`TempPathUnit`) ; toute nouvelle fixture qui nomme un
   fichier temporaire doit passer par `swingbot_test::uniqueTempName`
   (convention consignée ici — un check CI dédié serait disproportionné
   en mode maintenance).
4. *Notes /100* : **toutes inchangées** — Architecture 90, Qualité 94,
   FinTech 88, Production 78, **Rentabilité 28 (=)**. Le correctif D59
   consolide la fiabilité de la CI (moins de faux rouges) mais ne change
   ni le moteur ni sa couverture fonctionnelle ; pas de saut de note pour
   de l'hygiène de tests.

### Sprint 20 — Mode maintenance (2026-07-10)

**Contexte** : à l'ouverture, le Sprint 20 était « Décision requise » (trois
options mutuellement exclusives : (e) maintenance, (a'') finitions Production,
(r'') données alternatives §5.4). Une revue pré-sprint INDÉPENDANTE (fournie
par l'utilisateur) a rendu un verdict NO-GO explicite : aucun périmètre
exécutable tant que l'utilisateur n'a pas choisi, interdiction pour
l'exécuteur de trancher à sa place. Ses constats factuels ont été
contre-vérifiés un par un sur le dépôt (tous exacts ; une seule nuance :
les « finitions Production » sont des reliquats DOCUMENTÉS — D25/D52,
Sprint 17 — pas une incohérence de la ROADMAP). **Décision utilisateur :
option (e) — mode maintenance.** Le périmètre offline de recherche d'edge
est épuisé (8 familles/variantes soldées sans edge OOS depuis le Sprint 8) :
...
- [x] **20.1** Baseline réelle documentée AVANT toute modification (critère :
  build sans warning, `ctest -N` égal au tableau de bord, suite 100 % verte)
  → vérifiée 2026-07-10 (ci-dessus). Aucun défaut constaté, aucun correctif —
  donc aucun test rouge (en inventer un pour un sprint sans changement aurait
  été trompeur, conformément à la revue pré-sprint).
- [x] **20.2** Politique de maintenance écrite (`RUNBOOK.md` §8) → `15d5f24`
...
- [x] **(clôture)** D58 consignée : constats techniques de la revue
  ...
  conventions temporelles IBKR/Alpaca divergentes) — aucun défaut actif,
  rien à corriger en maintenance.

**Verdicts** : aucun (sprint sans mesure nouvelle). `config/prod.json`
intact, `liveTradingApproved` = false, verrou
`LiveTradingStaysDisapprovedUntilEdgeDoD` intact, goldens byte-identiques
...
3. *À détecter plus tôt / garde-fou* : les options (e)/(a'')/(r'') du tableau
   ...
   du « Sprint courant » : tant que le mode maintenance dure, il décrit
   l'ÉTAT stable et les CONDITIONS de réouverture (décisions préalables
   exigées par option), pas une liste d'items vide.
4. *Notes /100* : **toutes inchangées** — Architecture 90, Qualité 94,
   FinTech 88, Production 78, **Rentabilité 28 (=)** : aucun code, aucune
   mesure nouvelle ; le sprint acte l'arrêt de la recherche (la note ne
   bougera qu'à la réouverture du §5.4 ou sur décision contraire).

### Sprint 19 — Term-structure VIX/VIX3M : la dernière piste offline du §5 (2026-07-09)

**Contexte** : décision utilisateur (r') à l'ouverture = explorer la piste
§5.3 de `CONCLUSION_RECHERCHE_EDGE.md` (« le contango/backwardation comme
signal de régime — nécessite un export de données supplémentaire »), la
DERNIÈRE piste offline du §5 après les soldes §5.1/§5.2 du Sprint 18.
Décision utilisateur jointe : APPLIQUER le diff D54 (libssl-dev dans la
liste apt du prompt gouverné). Protocole complet : 3 pavages (canonique
750/500/500, fin 550/400/400, décalé offset 90), multi-univers, balayages,
grille resserrée (leçon 8t.3), MC size-aware (seed 42, D45), gardes
D33/D34/D35/D47, verdicts « sentinelle → mesure → figée ». Livrable annoncé :
un VERDICT verrouillé, pas une promesse de rentabilité.

**Baseline à l'ouverture** : **781/781 verte** (build −Werror sans warning,
Linux paquets système sans vcpkg, `ctest -N`=781, 613 unitaires + 168
intégration, aucune dérive — le tableau de bord était exact).

**Commits** (ordre chronologique = ordre d'exécution) :
- `6ece6d9` docs(workflow) : applique le diff D54 — libssl-dev dans la liste apt (décision utilisateur, Sprint 19)
- `04b91cb` feat(data) : export VIX3M_max.csv (indice ^VIX3M, term-structure — item 19.0)
- `d31e3f4` feat(term-structure) : moteur binaire VixTermRegimeBacktester + walk-forward (item 19.1)
- `aad65d7` test(term-structure) : verdict OOS verrouillé du filtre binaire (3 pavages, multi-univers, MC size-aware — item 19.2)
- `1897d75` feat(term-structure) : variante continue VixTermScaledBacktester (item 19.3)
- `f9c9765` test(term-structure) : verdict OOS verrouillé de la variante continue (item 19.4)
- `f88ea37` test(validation) : section 21 term-structure VIX/VIX3M + verdict figé (item 19.5)
- (clôture) docs : mise à jour roadmap + conclusion d'edge

**Items** :
- [x] **D54 (gouverné)** libssl-dev dans `prompt-executer-sprint.md` → `6ece6d9`
  Décision utilisateur explicite à l'ouverture ; commit dédié citant la
  décision (règle « pas d'auto-amendement » respectée : proposé au Sprint 17,
  appliqué seulement après décision).
- [x] **19.0** Export `VIX3M_max.csv` (porte de données) → `04b91cb`
  `scripts/export_total_return.py ^VIX3M` SANS modification (caret encodé,
  nom sans caret, fallback adjclose→close) : 5021 barres 2006-07-17 →
  2026-07-01 (END_DATE figée). Gate qualité : dates strictement croissantes,
  aucun close ≤ 0, Adj == Close partout (indice — ATTENDU, hors
  `auditTotalReturnCsv` comme VIX). Exception .gitignore miroir du Sprint 14.
- [x] **19.1** `include/backtest/VixTermRegimeBacktester.hpp` → `d31e3f4`
  Filtre BINAIRE : ratio VIX/VIX3M (0 = non valide → régime INCONNU → cash),
  SMA trailing `smoothLookback` (warmup = lookback − 1 ≤ 9 barres, D35
  trivial), long ssi ratio lissé ≤ `ratioThreshold`. TROIS séries alignées
  (`alignOnCommonDates` réutilisé tel quel, arité exigée 3), fonctions pures
  `tsRatio`/`tsSmaTrailing`. Coûts par côté D22, deployedFraction (D45).
  10 tests unitaires (P&L exacts à la main 1e-9, frontière ≤, retard du
  lissage, signal dégénéré).
- [x] **19.2** Verdict OOS binaire verrouillé → `aad65d7`
  8 verrous `VixTermRegimeOosIntegration` (axe, 3 pavages, MC, multi-univers,
  balayage, fragilité du candidat). Verdict D57 : AUCUN EDGE au seuil naturel ;
  le candidat 1,05 du balayage (PREMIER dSharpe > 0 du projet) est réfuté par
  la grille resserrée — verrou dédié `CandidateFragilityIsLocked`.
- [x] **19.3** `include/backtest/VixTermScaledBacktester.hpp` → `1897d75`
  Variante CONTINUE (leçon D55) : w = min(maxWeight, (VIX3M/VIX)^steepness),
  bande anti-churn `mmShouldRebalance` réutilisée, coût ∝ |Δw|, aucun warmup.
  Fonction pure `tsTargetWeight`. 9 tests unitaires.
- [x] **19.4** Verdict OOS continu verrouillé → `f9c9765`
  6 verrous `VixTermScaledOosIntegration`. Verdict D57 : dSharpe ≥ 0 partout
  mais ≤ 0,03 (bruit), alpha absolu négatif, quasi-B&H (poids 0,98) —
  AUCUN EDGE au sens de la DoD.
- [x] **19.5** Section 21 du harnais `validate` → `f88ea37`
  Imprime les MÊMES chiffres que les verrous (binaire + grille resserrée +
  continu + MC). Sections 1-20 inchangées.

**Verdicts (mesurés 2026-07-09, figés par test)** : voir D57. Le candidat du
balayage a été jugé par la confirmation hors-pavage (leçon 8t.1) AVANT tout
enthousiasme : positif sur les 3 pavages et les 4 actifs, il meurt sur la
grille resserrée (signe instable à ±0,01 de seuil sur le canon) et sur
l'alpha absolu (négatif partout) → gate de confirmation FERMÉ, AUCUNE
adoption. `config/prod.json` intact, `liveTradingApproved` = false, verrou
`LiveTradingStaysDisapprovedUntilEdgeDoD` intact, goldens byte-identiques.

**Tests** : 781 → **814/814 verts** (+19 unitaires, +14 intégration ; 632 + 182).
Sections 1-20 du `validate` inchangées ; section 21 ajoutée (verdicts
imprimés = verdicts verrouillés).

**Rétrospective** :
1. *Découpage* : bon — la porte de données 19.0 EN PREMIER (leçon Sprint 14) a
   levé la seule incertitude bloquante (fetch ^VIX3M : HTTP 200, 5021 barres)
   avant d'écrire le moindre moteur ; les items moteur→verrous→CLI se sont
   enchaînés sans surprise, chaque moteur additivement (aucun golden touché).
   Le D54 en tête de sprint (commit dédié) a évité de le retrouver en reliquat
   une 3e fois.
2. *Prompts du workflow* : suffisants, AUCUN diff proposé. La nouveauté du
   sprint (un candidat > 0 au balayage) était couverte par les leçons
   existantes (8t.1/8t.3 : confirmation hors-pavage + grille resserrée) — le
   protocole n'a pas eu besoin d'être improvisé, seulement appliqué.
3. *À détecter plus tôt / garde-fou* : l'axe 2006+ (VIX3M borne l'historique —
   pas de dot-com) était anticipé par l'exploration AVANT l'implémentation ;
   le caveat est consigné dans D57, les en-têtes des verrous et le verdict de
   la section 21. Garde-fou existant suffisant : le verrou `AxisIsLocked` fige
   les bornes de l'axe — toute dérive de données (ré-export plus court) casse
   le test. Rien de nouveau à ajouter au backlog.
4. *Notes /100* : **toutes inchangées** — Architecture 90, Qualité 94,
   FinTech 88, Production 78, **Rentabilité 28 (=)** : un verdict négatif de
   plus (le 8e) ne change ni la sûreté ni la capacité démontrée de gain ; la
   valeur du sprint est l'ÉPUISEMENT PROPRE du périmètre offline (§5 soldé
   sauf données nouvelles) et deux acquis méthodologiques : le mécanisme
   anti-cash-drag est confirmé, et le protocole a survécu à son premier
   candidat positif sans se laisser séduire.

### Sprint 18 — Réouverture de la recherche d'edge : Moreira-Muir + cointégration Engle-Granger (2026-07-09)

**Contexte** : décision utilisateur (r) à l'ouverture (demande : « le bot
est-il rentable à déployer ? trouve les moyens d'y parvenir ») = implémenter
les DEUX pistes de réouverture faisables offline listées par
`CONCLUSION_RECHERCHE_EDGE.md` §5 (données déjà dans le dépôt : `*_max.csv`,
`VIX_max.csv`), jugées par le protocole complet — 3 pavages (canonique / fin /
décalé offset 90), multi-univers / multi-paires, balayages, MC size-aware
(seed 42, D45), gardes D34/D35/D47, verdicts « sentinelle → mesure → figée ».
Livrable annoncé : un VERDICT verrouillé, pas une promesse de rentabilité.
Le reliquat D54 (diff gouverné libssl-dev) reste en attente de décision.

**Baseline à l'ouverture** : **732/732 verte** (build −Werror sans warning,
Linux paquets système sans vcpkg, `ctest -N`=732, 583 unitaires + 149
intégration, aucune dérive — le tableau de bord était exact).

**Commits** (ordre chronologique = ordre d'exécution) :
- `e9fbba7` feat(backtest) : scaling continu Moreira-Muir — VolScaledBacktester (item 18.1)
- `85584d5` feat(backtest) : scaling continu piloté par le VIX — VixScaledBacktester (item 18.2)
- `b9e76ce` feat(backtest) : briques Engle-Granger — OLS roulante causale + test DF (item 18.4)
- `1d8193b` feat(backtest) : pairs-trading cointégré Engle-Granger — CointPairsBacktester (item 18.5)
- `395b457` test(oos) : verdict Moreira-Muir verrouillé — AUCUN EDGE, le plus serré du projet (item 18.3)
- `0347f2a` test(oos) : verdict pairs cointégrés verrouillé — AUCUN EDGE, le gate filtre sans créer (item 18.6)

**Items** :
- **18.1** `include/backtest/VolScaledBacktester.hpp` — poids continu
  w = min(maxWeight, cible/vol annualisée réalisée), bande anti-churn
  (rebalance ssi |Δw| > bande), coût ∝ notionnel tradé |Δw| (D22), chaque coût
  porté par EXACTEMENT un stint (Σ pnl = équité), `TradeRecord` compatible MC
  size-aware, observabilité du churn (avgWeight/turnover/rebalanceCount).
  Réutilise `closeReturns`/`realizedVol` (zéro édition des headers golden).
  10 tests unitaires (P&L exacts à la main, hystérésis, coûts proportionnels).
- **18.2** `include/backtest/VixScaledBacktester.hpp` — variante vol IMPLICITE
  (w = cible/VIX, 2 séries alignées, décidable dès la barre 0 — le VIX est
  déjà une vol annualisée). Réutilise les fonctions pures du 18.1. 6 tests.
- **18.3** section 19 du harnais `validate` + 13 verrous
  (`VolScaledOosIntegration` ×7, `VixScaledOosIntegration` ×5 + série) —
  verdict D55 : AUCUN EDGE, mais dSharpe vs B&H ≈ 0 (le plus serré du projet).
- **18.4** `include/backtest/Cointegration.hpp` — OLS roulante CAUSALE
  (hedge ratio, jamais de données > end) + t-stat Dickey-Fuller lag 0 avec
  constante, critique Engle-Granger FIGÉE −3,34 en knob (simplification
  documentée dans le header). Math pur, aucune dépendance. 8 tests (1e-12).
- **18.5** `include/backtest/CointPairsBacktester.hpp` — gate de cointégration
  à l'ENTRÉE (β > 0 ET t ≤ critique), z-score du RÉSIDU, β figé à l'entrée
  (re-hedger chaque barre = churn non modélisé), poids w0 = 1/(1+β),
  w1 = β/(1+β) (brut 1,0, comparable au 0,5/0,5 naïf), garde d'activation
  `pctBarsCointegrated` (D47). 6 tests (gate fermé/ouvert, invariant comptable).
- **18.6** section 20 du harnais `validate` + 7 verrous
  (`CointPairsOosIntegration`) — verdict D56 : AUCUN EDGE, le gate filtre le
  bruit de D49 sans créer d'edge.

**Verdicts (mesurés 2026-07-09, figés par test)** : voir D55 et D56. Aucun
candidat → aucune confirmation hors-protocole D43 à lancer → AUCUNE adoption.
`config/prod.json` intact, `liveTradingApproved` = false, verrou
`LiveTradingStaysDisapprovedUntilEdgeDoD` intact, goldens byte-identiques.

**Tests** : 732 → **781/781 verts** (+30 unitaires, +19 intégration).
Sections 1-18 du `validate` inchangées ; sections 19-20 ajoutées (verdicts
imprimés = verdicts verrouillés).

**Rétrospective** : (1) la discipline « moteur séparé additif » a de nouveau
payé — deux familles explorées sans toucher un seul golden ; (2) le résultat
D55 est la première CONFIRMATION positive d'une hypothèse causale du projet
(le cash drag identifié en D50 était bien le mécanisme dominant : le corriger
comble l'écart de Sharpe) même si l'edge n'existe toujours pas — la
connaissance progresse par mécanismes, pas seulement par verdicts ; (3) le
piège classique du pairs-trading (trader un spread non testé) est désormais
documenté dans les DEUX sens : sans test = bruit (D49), avec test = trop rare
et toujours pas rentable (D56) ; (4) l'écriture des verrous en deux temps
(structure d'abord, pins après mesure) reste le bon geste — aucun chiffre
inventé n'a jamais touché un fichier de test.
...
**Contexte** : décision utilisateur (a) = parquer la prod paper et durcir
l'opérationnel (viser la note Production, backlog D18/D19/D25). L'exploration
d'ouverture a produit un constat majeur et honnête : **le backlog nommé du candidat
(a) était en grande partie PÉRIMÉ** (D52). D18 (ATR true-range) et D25 (drop de la
barre en formation) sont DÉJÀ implémentés et test-lockés ; D19 (lookback) est DÉJÀ
configurable (=230, coïncide avec le backtest) et re-priorisé hors runtime. De plus
les canaux d'alerte sont DÉJÀ E2E-testés (SMS + webhook). Le seul vrai manque
golden-safe restant était l'**observabilité** : `BotState` n'exposait aucune liveness
explicite, donc le healthcheck TCP détecte un process gelé mais pas un process qui
tourne SILENCIEUSEMENT malsain.

**Baseline à l'ouverture** : **708/708 verte** (build −Werror sans warning, Linux
paquets système, `ctest -N`=708, 567 unitaires + 141 intégration, aucune dérive).
Conteneur réutilisé (dépendances déjà installées).

**Commits** (ordre chronologique = ordre d'exécution) :
- `75dcdac` feat(observabilite) : instantané de santé structuré dans BotState (item 15.1)
- `e3eab73` feat(observabilite) : câblage santé dans le composition root + procédure RUNBOOK (item 15.2)
- (clôture) docs : mise à jour roadmap (recalage D18/D19/D25, D52)

**Tests** : 708 → **716** (+8 : 575 unitaires + 141 intégration). +8 BotStateHealthUnit
(défauts malsains ; âge dérivé du now injecté ; cas « silencieusement malsain » ; raison
kill-switch ; dé-auth Gateway ; borne d'horloge ; présence dans to_json_str ;
thread-safety concurrente). Suite nommée `BotStateHealthUnit` → couverte par le job TSan
ciblé (`WsServer|Watchdog|BotState`). **Goldens backtest byte-identiques** (BotState est
l'état dashboard/ops, hors chemin de trading).

**Changements** (tous golden-safe, aucune logique de trading touchée) :
- `include/core/bot_state.h` : instantané de santé — champs `last_cycle_healthy`,
  `last_healthy_cycle_epoch`, `gateway_authenticated`, `kill_switch_tripped` +
  `kill_switch_reason` ; setter `set_health` (un verrou/cycle) ; `health_json(now_epoch)`
  avec now INJECTÉ (teste `seconds_since_healthy_cycle` sans horloge murale, patron du seam
  `now_()` d'IBKRDataFeed) ; objet `health` ajouté à `to_json_str`.
- `include/bot/TradingBot.hpp` : `currentHaltReason()` — getter const pur (golden-safe)
  exposant l'état de NIVEAU du kill-switch (vs l'observateur sur front).
- `main_ibkr.cpp` : câblage — `set_health` appelé chaque cycle avec les faits déjà calculés
  (auth, cycle sain horodaté, kill-switch de niveau) avant le broadcast WS.
- `RUNBOOK.md` : section 4-bis — lecture de l'objet `health` (flux WS 9001), sens des champs
  et seuils d'action (`seconds_since_healthy_cycle > 4000`, dé-auth, kill-switch).

**Recalage du backlog (D52)** : D18/D19/D25 recalés « déjà résolu en code / re-priorisé »
avec les références fichier:ligne à l'appui (voir la table Découvertes). Dérive de type D20.

**Verdict de clôture** : DoD atteinte (716/716 verts, −Werror, goldens byte-identiques,
verrou live intact). Fichiers gouvernés (`config/prod.json`, `prompt-*.md`, CLAUDE.md
live-safety) intacts. `liveTradingApproved` = false. `DayTradeStrategy` dormante NON
supprimée (suppression = décision utilisateur). Décision de suite = **Décision requise
Sprint 16**.

**Rétrospective** :
1. *Découpage* : bon, mais l'exploration a RE-CADRÉ le sprint. Prévu « corriger D18/D19/D25 »,
   trouvé « ils sont déjà faits » → le sprint est devenu « fermer le vrai manque
   (observabilité) + recaler le backlog menteur ». 15.1 (code+tests TDD) → 15.2 (câblage +
   doc), additif. Le seam `now_epoch` injecté a rendu l'âge du cycle sain testable sans
   flakiness horaire — décision de conception clé.
2. *Prompts du workflow* : suffisants, AUCUN diff proposé. `AskUserQuestion`/`ExitPlanMode`
   encore instables cette session (ExitPlanMode a fini par passer au 2e essai) — la décision
   (a) est arrivée en texte libre, gérée sans toucher aux fichiers gouvernés.
3. *À détecter plus tôt / garde-fou* : **le backlog a menti** (D18/D25 marqués ouverts alors
   qu'ils étaient résolus en code — dérive D20 non attrapée). Garde-fou : l'étape 2 de
   `prompt-executer-sprint.md` recale déjà le DÉCOMPTE de tests sur `ctest -N` ; la leçon D52
   étend le principe aux DÉCOUVERTES — rouvrir un item de backlog commence par vérifier
   fichier:ligne qu'il est encore réel. Manque golden-safe résiduel noté au backlog Production
   (email SMTP E2E, CI lint/static-analysis/coverage, sonde `/healthz` HTTP, ré-auth Gateway).
4. *Notes /100* : **Production 72 → 74 (+2)** : l'observabilité comble un vrai trou (détection
   du « silencieusement malsain », au-delà du healthcheck TCP) et le RUNBOOK gagne une
   procédure ; modeste car la posture opérationnelle était déjà solide (d'où 72, pas 40).
   Architecture 90 (=), Qualité 93 (=) — le recalage du backlog est une victoire d'honnêteté
   documentaire, pas de couverture. FinTech 88 (=). **Rentabilité 28 (=)** : rien ici ne
   touche l'edge ni le verrou live.

### Sprint 14 — 5e famille (variante externe) : VIX-regime / vol implicite (2026-07-06)

**Contexte** : décision utilisateur (c') = variante de 5e famille à données EXTERNES,
après que le Sprint 13 (vol-regime sur vol RÉALISÉE) a été soldé sans edge. La seule
variante buildable dans un sprint est le régime de volatilité IMPLICITE (^VIX) — le
backlog D50 la nommait. On TRADE QQQ mais on gate l'exposition sur le NIVEAU du VIX
(vol implicite, anticipatrice, EXOGÈNE) vs sa médiane glissante, au lieu de la vol
réalisée dérivée du prix. Moteur SÉPARÉ (2 séries alignées QQQ+VIX, offline, ne touche
ni la prod ni aucun golden).

**Baseline à l'ouverture** : **694/694 verte** (build −Werror sans warning, Linux
paquets système sans vcpkg, `ctest -N`=694, décomposition réelle 560 unitaires + 134
intégration, aucune dérive). Conteneur réutilisé (dépendances déjà installées au
Sprint 13).

**Faisabilité des données (résolue)** : le VIX n'était PAS dans le dépôt. Vérifié :
le proxy egress de cet environnement AUTORISE `query2.finance.yahoo.com` (HTTP 200) —
le fetch passe (contrairement à la crainte initiale d'un blocage 403/407). TLS via
`SSL_CERT_FILE=/root/.ccr/ca-bundle.crt` (le proxy re-termine TLS). `export_total_return.py`
étendu (item 14.0) : encode le caret (^VIX → %5EVIX), nom de fichier sans caret,
fallback vers le close quand le bloc `adjclose` est absent (un indice sans dividende).
`VIX_max.csv` : 9192 barres 1990-01-02 → 2026-07-01, Adj Close == Close (attendu pour
un indice ; VIX volontairement HORS de la liste `auditTotalReturnCsv` — c'est un signal,
pas un actif total-return tradé).

**Commits** (ordre chronologique = ordre d'exécution) :
- `df13117` feat(data) : export VIX_max.csv (indice de vol implicite ^VIX, item 14.0)
- `7f1ddaf` feat(vix-regime) : moteur VixRegimeBacktester + VixRegimeWalkForward (item 14.1)
- `730f283` test(vix-regime) : verdict OOS verrouillé (item 14.2)
- `c8b78e6` test(validation) : section 18 famille vix-regime + verdict figé (item 14.3)
- (clôture) docs : mise à jour roadmap

**Tests** : 694 → **708** (+14 : 567 unitaires + 141 intégration). +7
VixRegimeBacktesterUnit (arité 2 exigée, VIX plat = calme → long, seuil<1 gate le
régime en cash (D33), VIX baissier → long P&L exact sur QQQ, VIX haussier → cash plat,
deux stints + cash plat entre (D47), fenêtre < warmup) ; +7 VixRegimeOosIntegration
(axe QQQ∩VIX verrouillé 6870 barres, 3 pavages, MC size-aware graine fixe, multi-univers
4 actifs gatés sur le MÊME VIX, balayage). **Goldens backtest byte-identiques** (moteur
séparé).

**Interfaces ajoutées** (additives, header-only) : `include/backtest/VixRegimeBacktester.hpp`
— `VixRegimeConfig` (refLookback, thresholdMult, coûts — PAS de volLookback : le VIX EST
le signal, D33), `VixRegimeResult` (miroir de VolRegimeResult), `VixRegimeBacktester`
(2 séries alignées via `alignOnCommonDates`, seam `fromAxis`, run/runRange/compute_),
`VixRegimeWalkForward`. Réutilise `trailingMedian` (VolRegimeBacktester),
`alignOnCommonDates`/`AlignedAxis` (RotationBacktester), `TradeRecord`, `MonteCarlo`,
`CsvDataFeed`. Warmup réduit à refLookback−1 (le VIX est déjà un niveau de vol).
Section 18 du CLI `validate`.

**Verdict OUT-OF-SAMPLE** :
- **La famille TRADE massivement** (D47) : 195-227 stints OOS (plus que le VolRegime :
  le VIX croise sa médiane plus souvent que la vol réalisée, plus lisse).
- **MAIS AUCUN EDGE ajusté du risque** : Sharpe stratégie POSITIF (0,55 canon / 0,42
  fin / 0,45 décalé) mais SOUS le Sharpe du B&H (1,06 / 0,87 / 0,98) → dSharpe < 0 sur
  les 3 pavages ; idem sur les 4 actifs gatés sur le MÊME VIX (QQQ 0,42, SPY −0,01,
  IWM 0,38, MDY 0,28, tous < leur B&H) et les 9 réglages (meilleur rl=126/seuil=1,2 =
  −0,22).
- **Constat (D51)** : la vol IMPLICITE (anticipatrice, exogène) ne fait PAS mieux que la
  vol réalisée du Sprint 13. DD réduit (13,4 vs 18,6 %) mais < 50 %, alpha vs B&H négatif.
  Sortir du marché en régime de vol haute (réalisée OU implicite) coûte plus en rendement
  manqué que ça ne rapporte en risque réduit, sur un QQQ structurellement haussier. MC
  size-aware cagrP50 +5,15 %, DD p95 40,3 %.
- Axe QQQ∩VIX = 6870 barres (borné par QQQ 1999 ; le VIX ~1990 déborde).

**Verdict de clôture** : DoD NON atteinte (dSharpe OOS négatif partout) → prod paper,
`liveTradingApproved` = false, verrou `LiveTradingStaysDisapprovedUntilEdgeDoD` intact.
Fichiers gouvernés intacts. **Les DEUX variantes de la 5e famille (vol réalisée + vol
implicite) sont soldées sans edge OOS.** Décision de suite = **Décision requise Sprint 15**.

**Rétrospective** :
1. *Découpage* : bon, et un item de DONNÉES en tête (14.0) proprement isolé comme gate.
   La grande incertitude (fetch Yahoo bloqué ou non) a été levée EN PREMIER par un test
   de reachability read-only (HTTP 200) avant d'écrire le moteur — pas d'engagement à
   l'aveugle. 14.1 (moteur 2-séries + unit tests) → 14.2 (verrou OOS) → 14.3 (CLI),
   additif. Réutilisation massive du VolRegimeBacktester (Sprint 13) : `trailingMedian`,
   coûts, métriques, MC size-aware — seul l'ingest 2-séries et le swap de signal sont
   neufs, comme PairsBacktester a réutilisé RotationBacktester. Aucune dépendance ratée.
2. *Prompts du workflow* : suffisants, AUCUN diff proposé. La décision (c') et la
   sous-variante (VIX, seule faisable offline) ont été tranchées par la faisabilité +
   décision utilisateur. `AskUserQuestion` ET `ExitPlanMode` sont restés en panne cette
   session (« permission stream closed » ; ExitPlanMode a fini par passer à la 2e
   tentative) — géré sans jamais improviser sur les fichiers gouvernés ni router autour
   du proxy egress.
3. *À détecter plus tôt / garde-fou* : le fetch d'une donnée EXTERNE introduit deux
   risques que le reste du projet (offline) n'a pas — egress policy et format d'index
   (pas d'adjclose). Anticipés : reachability testée d'abord, fallback `adjclose`→`close`
   codé, `SSL_CERT_FILE` documenté. Garde-fou de fait : le VIX est tenu HORS de la liste
   `auditTotalReturnCsv` (un indice DOIT avoir Adj==Close ; l'y inclure ferait crier le
   garde-fou D29 à tort). Candidat backlog (D51) : le scaling continu (Moreira-Muir strict
   sur le VIX) et la term-structure (VIX/VIX3M) restent non jugés — le premier jet ne juge
   que le filtre binaire sur le niveau du VIX spot.
4. *Notes /100* : Architecture 90 (=) : 4e moteur offline, réutilise l'ingest 2-séries et
   l'infra vol du Sprint 13 sans les dupliquer. Qualité 93 (=). FinTech 88 (=) : une
   hypothèse FinTech légitime (vol-managed sur vol IMPLICITE) correctement jugée, sans
   edge. Production 72 (=). **Rentabilité 28 (=)** : la variante externe de la 5e famille
   est jugée et soldée sans edge ; la capacité démontrée à gagner de l'argent est
   inchangée, mais la connaissance négative se renforce (on SAIT que ni la vol réalisée ni
   la vol implicite ne donnent d'edge ajusté du risque net de coûts sur ETFs US).
