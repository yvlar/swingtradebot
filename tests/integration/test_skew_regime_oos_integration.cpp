...
//  Sprint 22 (item 22.1) — VERDICT OUT-OF-SAMPLE VERROUILLÉ.
//
//  Chantier (r'') « données alternatives / surface d'options » (§5.4, décision
//  utilisateur 2026-07-11) : le signal n'est plus le NIVEAU de la vol implicite
//  (^VIX, Sprint 14) mais le PRIX DU RISQUE DE QUEUE — l'indice CBOE ^SKEW,
//  dérivé du smile des options SPX hors de la monnaie. Hypothèse (a) du sprint :
//  quand la queue est CHÈRE (SKEW haut vs sa propre médiane glissante), le marché
//  d'options price un krach → cash ; queue bon marché → long. Le moteur du
//  Sprint 14 est générique {actif tradé, indice signal} : AUCUN nouveau moteur.
//
...
//  total-return (*_max.csv) + SKEW_max.csv (9117 barres 1990+ → axe borné par QQQ
//  1999+, dot-com et 2008 COUVERTS — contrairement au VIX3M du Sprint 19).
//  3 pavages (canonique/fin/décalé, D36). Warmup = refLookback − 1 = 125 (D35).
//
//  NOTE balayage : la plage utile de thresholdMult diffère du VIX — le SKEW est
//  borné (~100-183) et s'écarte rarement de ±10 % de sa médiane ; des seuils
//  0,8/1,2 seraient toujours-cash/toujours-long (garde D47 violée). On balaye
//  {0,95 ; 1,00 ; 1,05 ; 1,10}.
//
//  Valeurs MESURÉES le 2026-07-11 sur QQQ_max ∩ SKEW_max et figées.
// ============================================================
...
// Config EXPLICITE (D33). Médiane de référence du SKEW sur 126 j (~6 mois), seuil 1,0
// (queue plus chère que sa médiane → cash) — miroir du réglage naturel du Sprint 14.
...
// ════════════════════════════════════════════════════════════
//  Axe commun QQQ ∩ SKEW verrouillé (le SKEW ~1990 déborde QQQ ~1999 → borné par
//  QQQ, comme le VIX du Sprint 14 : dot-com et 2008 SONT dans l'axe).
// ════════════════════════════════════════════════════════════
...
TEST(SkewRegimeOosIntegration, SkewRegimeCanonicalPavingVerdictIsLocked) {
    ...
    // Verdict : AUCUN edge ajusté du risque — PIRE que le régime VIX du Sprint 14
    // (dSharpe −0,68 contre −0,52) : le prix de queue lissé est un signal PLUS
    // bruité que le niveau de vol, il fait churner le filtre (343 trades contre
    // 216). DD réduit (16,6 vs 18,9) mais < 50 %, alpha très négatif (cash drag).
...
TEST(SkewRegimeOosIntegration, SkewRegimeShiftedPavingVerdictIsLocked) {
    ...
    // 11 fenêtres (12 sur l'axe VIX 6870) : l'axe SKEW fait 6797 barres, une
    // fenêtre décalée de moins y tient — attendu, verrouillé par AxisIsLocked.
...
TEST(SkewRegimeOosIntegration, SkewRegimeMonteCarloSizeAwareIsLocked) {
    ...
    // cagrP50 1,6 % : quasi rien de la hausse de QQQ n'est capté, et la queue de
    // drawdown (ddP95 62,5 %) est PIRE que le régime VIX (40,3) — le churn du
    // signal SKEW concentre les pertes. Cohérent avec le verdict des pavages.
...
//  (prix de queue du marché SPX). Le régime SKEW aide-t-il un actif quelconque ?
// ════════════════════════════════════════════════════════════
...
TEST(SkewRegimeOosIntegration, SkewRegimeThresholdSweepBestOosIsLocked) {
    ...
    const double mults[] = {0.95, 1.0, 1.05, 1.1};
    ...
    // VERDICT 22.1 (figé) : AUCUN des 12 réglages ne bat le B&H sur le Sharpe —
    // le meilleur (refLookback=63 / seuil=1,10, le réglage qui trade le MOINS :
    // 91 A/R) reste à dSharpe −0,08. Monotone : plus le filtre est permissif,
    // moins il perd — la limite « toujours long » EST le B&H. Aucun candidat
    // dSharpe > 0 → pas de grille resserrée (8t.3 sans objet), gate de
    // confirmation FERMÉ.
    EXPECT_NEAR(best, -0.0769, 1e-2);
    EXPECT_EQ(bestR, 63);
    EXPECT_DOUBLE_EQ(bestM, 1.1);
